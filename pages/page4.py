...
def actualizar_graph(fecha_min, fecha_max, selected_player, selected_team):
    ...
    player_data["hover_text"] = (
        ...
        "Intentos: " +player_data["3FGM"].astype(str)  +"/" +player_data["3FGA"].astype(str) +" - " + (player_data["3FG_pct_01"] * 100).round(1).astype(str) + " %" +"<br>" 
    )
...
@callback(Output('graph_line11', 'figure'),
          [Input('selector_fecha', 'start_date'),
           Input('selector_fecha', 'end_date'),
           Input('dropdown-player', 'value'),
           Input('dropdown-team', 'value')])
...
def actualizar_graph_1(fecha_min, fecha_max, selected_player,selected_team):
